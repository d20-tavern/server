            ...
            match &attr.parse_meta().map_err(|err| err.to_compile_error())? {
                ...
                Meta::NameValue(nv) => {
                    match nv.path.get_ident() {
                        None => return Err(compile_error_args!(nv.span(), "name_value attributes should not contain paths")),
                        Some(id) => {
                            match id.to_string().as_str() {
                                "table_name" => {
                                    if table.is_some() {
                                        return Err(compile_error_args!(nv.span(), "table_name attribute should only be set once"));
                                    }
                                    let lit = literals::lit_to_lit_str(&nv.lit)?;
                                    table = Some(literals::try_from_lit_str(&lit)?);
                                },
                                _ => return Err(compile_error_args!(nv.span(), "unknown attribute name {}", id)),
                            }
                        }
                    }
                },
                Meta::Path(path) => return Err(compile_error_args!(path.span(), "unknown attribute {:#?}", path)),
            }
        }

        Ok(DBStructAttrs{ table, id_field: id_field.unwrap_or(literals::try_from_lit_str(&LitStr::new("id", Span::call_site()))?) })
    }
}