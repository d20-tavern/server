...
pub trait GetById {
    fn get_by_id(parent_id: Option<Uuid>, this_id: Uuid) -> BoxedFilter<(Box<dyn Reply>,)>;
}

impl<T> GetById for T where T: db::GetById + Serialize + StatusInnerData {
    fn get_by_id(parent_id: Option<Uuid>, this_id: Uuid) -> BoxedFilter<(Box<dyn Reply>,)> where Self: Sized + Send {
        async fn get_item_by_id<U>((conn, this_id): (Connection, Uuid)) -> Result<Box<dyn Reply>, Rejection> where U: db::GetById + Serialize + StatusInnerData {
...
impl<T> GetAll for T where T: db::GetAll + Serialize + StatusInnerData {
    fn get_all(parent_id: Option<Uuid>) -> BoxedFilter<(Box<dyn Reply>,)> where Self: Sized + Send {
        async fn get_all_items<U>((conn, parent_id): (Connection, Option<Uuid>)) -> Result<Box<dyn Reply>, Rejection> where U: db::GetAll + Serialize + StatusInnerData {
...
impl<T> Insert for T where T: db::Insert + forms::TryFromForm + 'static {
    fn insert(parent_id: Option<Uuid>) -> BoxedFilter<(Box<dyn Reply>,)> {
        async fn insert_item<U>((conn, form, parent_id): (Connection, Form, Option<Uuid>)) -> Result<Box<dyn Reply>, Rejection> where U: db::Insert + forms::TryFromForm + 'static {
...
pub trait Update {
    fn update(parent_id: Option<Uuid>, this_id: Uuid) -> BoxedFilter<(Box<dyn Reply>,)>;
}
...
impl<T> Update for T where T: db::Update + forms::TryFromForm + 'static {
    fn update(parent_id: Option<Uuid>, this_id: Uuid) -> BoxedFilter<(Box<dyn Reply>,)> {
        ...
        warp::filters::method::put()
            .and(nebula_form::form_filter())
            .and(db::conn_filter())
            .map(move |form, conn| (conn, form, this_id, parent_id))
...
pub trait DeleteById {
    fn delete_by_id(parent_id: Option<Uuid>, this_id: Uuid) -> BoxedFilter<(Box<dyn Reply>,)>;
}
...
impl<T> DeleteById for T where T: db::DeleteById + 'static {
    fn delete_by_id(parent_id: Option<Uuid>, this_id: Uuid) -> BoxedFilter<(Box<dyn Reply>,)> {
        ...
        warp::filters::method::delete()
            .and(db::conn_filter())
            .map(move |conn| (conn, this_id, parent_id))
